    ...
    <div class="Filters-in" :ref="groupName" v-else>
      ...
      <ul class="Filters-first" v-if="groups.length > 0">
        ...
        <template v-for="group in groupedCheckboxes">
          <div class="Filters-head border-bottom mb-3 mt-1 ">
            <h5>{{ group.title }}</h5>
          </div>
      ...
      <button type="button" @click="checkboxClose(groupName)" class="Filters-in-m-button">TAMAM <span
          v-if="selectedLength">({{ selectedLength }})</span></button>
    </div>
